...
?>
<h2><?php _e('Image setting for','wpc') ?> <?=$title?></h2>
        ...
        <?php if ($variation_attribute_found) {
            for($i=0;$i<count($base_attribute);$i++){ ?>
                ...
                <?php  foreach ($attributes as $attribute) {
                      ...
                      <?php  foreach ($post_terms as $term) { ?>
                          <tr>
                              ...
                              <td style="text-align: center">
                                  <?php $checkedValue=@$image_config['_wpc_images'][$base_attribute[$i]->slug][$attribute['name']][$term->slug]["no_image"]==1?"checked":"";?>
                                  <input <?=$checkedValue?> type="checkbox" value="1" name="_wpc_images[<?=$base_attribute[$i]->slug?>][<?=$attribute['name']?>][<?=$term->slug?>][no_image]">
                              </td>
                          </tr>
                            <?php }?>
                    </table>
                    ...
                    <?php }?>
            <?php }}?>
            <button type="submit" class="button button-primary" id="icBtn"><?php _e('Save Images','wpc') ?></button>
        </form>
